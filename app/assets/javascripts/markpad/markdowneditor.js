$(function () {
  //filepicker
	filepicker.setKey('A5ISNG13WS9-zDdAiKJn5z');
	...
	$.markdownEditor = {
		buttons: {
			'open' : {'alt': 'Open', 'name': '<i class="icon-folder-open icon-white"></i> Open', 'icon':'', 'btn_class':'btn-primary', 'icon_class':'icon-white', callback: function(){}},
	...
	$.markdownEditor.ui.prototype.makeButton = function(key, obj) {
		var button = $("<button></button>").addClass('me-'+key).addClass('btn').attr('alt', (obj.alt ? obj.alt : obj.name)).click(this.clickHandler(obj.callback));
	...
	$.markdownEditor.ui.prototype.clickHandler = function(callback) {
		var that = this;
		return function (e) { callback.apply(null, [that.$textarea.caret(), that.$textarea]); that.$textarea.preview(); return false;};
	}
	...
	$(".me-savemd").click(function(){
		...
		filepicker.saveAs(dataUrl, 'md', function(dataUrl, dataToken, data) {if (typeof console !== 'undefined' && typeof console.log !== "undefined"){console.log("save md as " + dataUrl + " " + dataToken + " " + data);}});
	});
	$(".me-savehtml").click(function(){
		var dataUrl = filepicker_utilities.getUrlFromData($("#preview").html());
		filepicker.saveAs(dataUrl, 'html', function(dataUrl, dataToken, data) {if (typeof console !== 'undefined' && typeof console.log !== "undefined"){console.log("save html as " + dataUrl + " " + dataToken + " " + data);}});
	});
	}
	$.markdownEditor.filepicker.save = function(){
		...
		var dataUrl = filepicker_utilities.getUrlFromData($("#editor").val());
		filepicker.save(dataUrl, function(){if (typeof console !== 'undefined' && typeof console.log !== "undefined"){console.log("saved");}});
		}
	}
